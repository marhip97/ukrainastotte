...
| 2026-04-22 | M4: Dashboard MVP | Pågår | GitHub Pages-deploy workflow lagt til (`.github/workflows/deploy-pages.yml`). Bygger `_site/` med dashboard-filer og processed-data, injiserer deploy-stier i index.html, publiserer via `actions/deploy-pages`. Trigges ved endring i `src/dashboard/`, `data/processed/` eller workflow-filen. Simulert build verifisert lokalt - alle fem ressurser leveres 200. README oppdatert med Pages-URL. | devops | Aktivere Pages med Source=GitHub Actions i repo-innstillinger. |
